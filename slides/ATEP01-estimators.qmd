...
data_angles = generator.rvs(size=1000)
...
def funcU(args: tuple) -> float:
    return UnbinnedNLL(data_angles, tuple(args))
...
for alpha in alpha_grid:
    ...
    for beta in beta_grid:
        La.append(UnbinnedNLL(data_angles, (alpha, beta)))
    L.append(La)
...
In case $\nu$ and $\vec{\theta}$ are independent we can find the estimator $\widehat{\vec{\theta}}$ by maximizing the likelihood:

$$\ln {\mathcal{L}(\vec{\theta})} = n\ln \nu -\nu + \sum_{i=1}^n \ln f(x_i;\vec{\theta}) + C \rightarrow \frac{\partial \ln {\mathcal{L}(\vec{\theta})}}{\partial \theta_i}\Bigr|_{\theta_i = \hat{\theta}_i} = 0$$
...
# Generate a poisson random number of background events
bg_events = np.random.poisson(truth.mu_b)
background = np.random.exponential(truth.tau, bg_events)

# Generate a poisson random number of signal events events
sig_events = np.random.poisson(truth.mu_s)
...
ax.scatter(sample, 
        ...
        s = 500, marker = '|') 
ax.plot(x, truth.mu_b * 
        expon(0,truth.tau).pdf(x), 
        lw=2, linestyle=":", color="black")
ax.plot(x, truth.mu_s * 
        norm(truth.mu, truth.sigma).pdf(x), 
        lw=2, color="blue")
...
```
:::{.callout-note}
We are assuming that we know the true parameters for the gaussian and background, the only unknowns are the amount of signal and background events in the sample, ie $\mu_s, \mu_b$.
:::

## Extended Maximum Likelihood {.smaller}

Now we are scan the likelihood function and find its minimum. 
...
L = np.array(L)
i_best = np.where(L == L.min())
fitted_ext = Namespace(mu_s = np.float64(mu_s_grid[i_best[0]]), mu_b = np.float64(mu_b_grid[i_best[1]]))
print (f"Signal estimate: {fitted_ext.mu_s:.2f}")
print (f"Background estimate: {fitted_ext.mu_b:.2f}")
```
...
plt.clabel(cs, cs.levels, inline=1, 
    fmt=fmt, fontsize=20)
plt.scatter(fitted_ext.mu_b, fitted_ext.mu_s, 100,
    marker='o',color='w', label="Estimated")
plt.scatter(truth.mu_b, truth.mu_s, 100,
    marker='+',color='r', label="Truth")
cbar = plt.colorbar(mesh)
cbar.ax.set_ylabel('-$\ln \mathcal{L}$')
plt.legend()
...
* The following is plot of estimates of $\hat{\mu}_s$ done for a true $\mu_s$ = 2.5. As can be seen in many instances the estimate is negative (We will see more on this when discussing confidence intervals)
...
* The number of events in the bin-$i$ based on the unknown parameters, $\vec{\theta}$ is then defined as:
  $$\nu(\vec{\theta})_i = n_{tot}\int_{x^i_{min}}^{x^i_{max}}f(x;\vec{\theta}){\rm d}x$$ 
  where $x^i_{min}$, $x^i_{max}$ are the limits of the bin-$i$. 
* We have now a vector $\vec{\nu}=(\nu_1,... \nu_N)$ of $N$ bins of expected values per bin, plus a vector $\vec{n} = (n_1, ... n_N)$ as our data given by the measured content per bin.
 
 

## Binned Maximum Likelihood {.smaller}

We can model density probability function of our data as a multinominal distribution:
...
:::{.callout title="Example"}
Let's go back to the decay of a particle, where we are going to binned our data in 5 bins. 
:::
...
ax.axvline(x = tau_grid[i_best], 
    color='k', linestyle=":", 
    label=r"$\hat{\tau}$ = "+f"{tau_grid[i_best][0]:.2f}")
ax.set_xlabel(r"$\tau$")
...
* The ML method **does not** give you a goodness-of-fit measure.
* If the assume *pdf* $f(x;\vec{\theta})$ is wrong and not capable of describing the data, the ML will still find a $\hat{\vec{\theta}}$, even if the fit is terribly wrong
* The reason for this is that the **absolute scale** of the likelihood function tells us nothing about how good the fit is, we only care for the maximum. 
...
In the case of fitting a histogram to a model, we can also consider the problem as a series of $\vec{y}$ values (the bin content) related to the $\vec{x}$ varible. If the bin content is large enough we can assume that the errors on $y_i$ are Gaussian. So we can assume that each value is a r.v from a Gaussian distribution with unknown $\mu_i$ but known $\sigma_i$.

:::{.callout-note title="Example"}
In the example on the right, we take the scattering angle data and plot it as $y$ vs $x$ plot, assuming the error of each bin is given by $\sqrt{y}$.
:::
...
hist, bin_edges = np.histogram(data_angles)
bincenters = bin_edges[:-1] + np.diff(bin_edges)/2
ax.errorbar(bincenters, hist, 2*np.sqrt(hist), 
    label="Data",fmt='o', ms = 10, markeredgecolor="white")
ax.set_xlim(-1,1)
ax.set_ylim(0,200)
ax.set_xlabel(r"$\cos(\theta)$")
ax.set_ylabel(r"content")
...
The $N$ measurements of $y_i$ can be equivelent regarded as a single measurement where the r.v. $\vec{y}$ follows a N-dimensional Gaussian. 

$$ g(\vec{y};\vec{\lambda},\vec{\sigma}) = \prod_{i=1}^N\frac{1}{2\pi\sigma_i}\exp\left(\frac{-(y_i - \lambda_i)^2}{2\sigma_i^2}\right)$$

In this case we have $N$ true values $\vec{\lambda}$, however consider the case in which we want to fit a function, so the true values are given as a fucntion of $x$, $\lambda(x;\vec{\theta})$.

:::{.callout-note title="Example"}
In our previous example of the scattering angle, the function was given by:

$$ \lambda(x;\alpha, \beta) = \frac{1 + \alpha x + \beta x^2}{2 + 2\beta/3}$$

where in this case $\vec{\theta} = (\alpha, \beta)$.
:::

## Relation of ML and $\chi^2$ {.smaller}

The log-likelihood then becomes (ignoring factors that do not depend on $\vec{\theta}$):
$$\ln \mathcal{L}({\vec{\theta}}) = -\frac{1}{2}\sum_{i = 1}^N \frac{(y_i - \lambda(x_i;\vec{\theta}))^2}{\sigma_i^2}$$
...
:::{.callout-important title="Least Squared"}
Note that this the chi-squared function and the parameters $\hat{\theta}_i$ that minimize this function are called **Least Squared** (LS) estimators.  
:::


## Relation of ML and $\chi^2$ 

If the parameters are correlated, ie, and the covariance matrix is known, the exponent becomes:

$$\chi^2(\vec{\theta}) = \sum_{i,j=1}^N (y_i - \lambda(x_i;\vec{\theta}))^2 (V^{-1})_{i,j} (y_j - \lambda(x_j;\vec{\theta}))^2$$

:::{.callout-note title="Least Squared"}
...
One can apply the least square method to any function $\lambda(x;\vec{\theta})$ but it has interesting properties when the function is linear combination of the parameters $\vec{\theta}$.
...
1. the $y_i$ are independent Gaussian r.v.
2. the hypothesis  $\lambda(x;\vec{\theta})$ is linear in the parameters $\vec{\theta}$
3. the functional form of the hypothesis is correct

Then the minimum value of $\chi^2$ is distributed with tje $\chi^2$ distribution, $f_{\chi^2}(x;k)$, with $k$ degres of freedom defined as:

$$f_{\chi^2}(x; k) = \begin{cases} \frac{1}{2^{k/2}\Gamma(k/2)} x^{k/2 - 1} e^{-x/2} & \text{for } x \geq 0 \\ 0 & \text{otherwise} \end{cases}$$ 

 
::: {.callout-note}
The degrees of freedom, $k$ is defined as $k$ = *number of points - number of fitted parameters*
:::
 

## Goodness-of-Fit {.smaller}

The expected value of $\chi^2$ under $f_{\chi^2}(x;k)$ is $k$.
$$ E[\chi^2] = k$$ 
So if $\chi^2_{min} \approx k$ we say the fit is good. More correctly to assess the goodness-of-fit we can calculate the $p$-value as:

  $$ p = \int_{\chi^2_{min}}^{\infty}f_{\chi^2}(x;k){\rm d}x$$
...
Let's imagine you obtained a $\chi^2_{min} = 2.4$ and the degrees of freedom is $k=4$
...
# for fill_between
pb=np.arange(2.4,20,0.01)

ax.set_ylim(0,0.2)
ax.fill_between(pb,ch.pdf(pb),alpha=0.5, color='g', label=f"$p$-value = {ch.sf(2.4):.2f}")


ax.vlines(x = 2.4,ymin=0, ymax=ch.pdf(2.4), color="black", linestyle="--")
